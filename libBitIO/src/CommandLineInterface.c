...
extern "C" {
    ...
    static void DisplayCLIHelp(CommandLineInterface *CLI) {
        if (CLI == NULL) {
    ...
    static void DisplayProgramBanner(CommandLineInterface *CLI) {
        if (CLI == NULL) {
    ...
    void SetCLISwitchAsChild(CommandLineInterface *CLI, const uint64_t ParentSwitch, const uint64_t ChildSwitch) {
        if (CLI == NULL) {
            Log(LOG_ERR, "libBitIO", "SetCLISwitchAsChild", "Pointer to CommandLineInterface is NULL\n");
        } else if (ParentSwitch > CLI->NumSwitches) {
    ...
    uint64_t GetCLISwitchNumFromFlag(CommandLineInterface *CLI, const char *Flag) {
        ...
        if (CLI == NULL) {
            Log(LOG_ERR, "libBitIO", "GetCLISwitchNumFromFlag", "Pointer to CommandLineInterface is NULL\n");
        } else {
            for (uint64_t Switch = 0; Switch < CLI->NumSwitches; Switch++) {
