...
typedef struct CRC16_CCITT {
	uint16_t Polynomial        = 0x8810;
	uint16_t Initialization    = 0xFFFF;
	size_t   CRCSize           = 16;
	bool     InputIsReflected  = false;
	bool     OutputIsReflected = false;
	uint16_t XOROutput         = 0x0000;
} CRC16_CCITT;

typedef struct CRC16_CDMA2000 {
	uint16_t Polynomial        = 0xE433;
	uint16_t Initialization    = 0xFFFF;
	size_t   CRCSize           = 16;
	bool     InputIsReflected  = false;
	bool     OutputIsReflected = false;
...
typedef struct CRC16_USB { // low byte first, aka little endian
	uint16_t Polynomial        = 0xC002;
	uint16_t Initialization    = 0xFFFF;
	size_t   CRCSize           = 16;
	bool     InputIsReflected  = true;
	bool     OutputIsReflected = true;
	uint16_t XOROutput         = 0xFFFF;
} CRC16_USB;

typedef struct CRC32 {
	uint32_t Polynomial        = 0x82608EDB;
	uint32_t Initialization    = 0xFFFFFFFF;
	size_t   CRCSize           = 32;
	bool     InputIsReflected  = true;
	bool     OutputIsReflected = true;
	uint32_t XOROutput         = 0xFFFFFFFF;
} CRC32;
